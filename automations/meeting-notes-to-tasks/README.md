...
- open [n8n Drive setup](https://n8n-production-192e.up.railway.app/webhook/drive-setup), drag **Send this Doc to HQ Tasks** to the bookmarks bar, open a Gemini Doc on `docs.google.com`, click the bookmark — or paste the Doc URL **and** the notes text on that page (private Docs; no sharing change), or
...
2. Send that header only to `/webhook/meeting-notes`.
...
- HQ confirmation task: paste that same Doc URL into **Drive URL** (or the file id into **Drive file ID**). n8n polls it every minute, exports if it is Anyone with the link, and writes HQ Tasks. Private Docs still need Drive-setup paste or Apps Script.
...
Apps Script also sends `googleAccessToken` (`ScriptApp.getOAuthToken()`). `{ "text": "..." }` alone on the Header Auth path becomes `Drive file ID = inline-*` (dry-run only).
