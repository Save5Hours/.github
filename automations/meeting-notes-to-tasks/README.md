...
- paste from [n8n `/webhook/apps-script-source`](https://n8n-production-192e.up.railway.app/webhook/apps-script-source) into [script.google.com](https://script.google.com), set `WEBHOOK_SECRET_PASTE` to the n8n Header Auth value, and run **`verifyDrivePath`** (walks Meet Recordings subfolders; no GCP client).
...
- Deploy `scripts/apps-script-drive-webhook.js` bound to the Workspace account. Copy it from [n8n `/webhook/apps-script-source`](https://n8n-production-192e.up.railway.app/webhook/apps-script-source). Paste the n8n Header Auth value into `WEBHOOK_SECRET_PASTE` (or Script property `WEBHOOK_SECRET`). Run **`verifyDrivePath` once** — it creates a real Google Doc, POSTs `{ fileId, text }` to n8n, and installs the 1-minute trigger. Keep the n8n **Webhook** node published.
